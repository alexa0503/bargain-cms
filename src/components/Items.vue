      ...
      <v-card>
        ...
        <v-data-table :loading="loading" :headers="headers" :items="items" hide-actions>
          ...
          <template slot="items" slot-scope="props">
            ...
            <td>{{ props.item.is_posted == 1 ? '已发布' : '已下架' }}</td>
            <td>{{ props.item.created_at }}</td>
            <td class="justify-center  px-0">
              <v-icon title="编辑" small class="mr-2" @click="editItem(props.item)">
                fas fa-pen
              </v-icon>
              <v-icon v-if="props.item.is_posted == 0" class="mr-2"  title="发布" small @click="publishItem(props.item.id,'on')">
                fas fa-toggle-off
              </v-icon>
              <v-icon v-if="props.item.is_posted == 1" class="mr-2"  title="下架" small @click="publishItem(props.item.id,'off')">
                fas fa-toggle-on
              </v-icon>
              <v-icon v-if="props.item.is_posted == 0" title="删除" small @click="deleteItem(props.item)">
                fas fa-trash-alt
              </v-icon>
            </td>
          </template>
    ...
    data() {
      return {
        ...
        headers: [{
          ...
          {
            text: '最低砍价金额',
            value: 'bargain_min_price',
            sortable: false
          },
          {
            text: '最高砍价金额',
            value: 'bargain_max_price',
            sortable: false
          },
          {
            text: '最少砍价次数',
            value: 'bargain_min_times',
            sortable: false
          },
          {
            text: '最多砍价次数',
            value: 'bargain_max_times',
            sortable: false
          },
          {
            text: '创建时间',
